...
static int send_aoa(struct mux_connection *conn, const unsigned char *data, int length)
{
	...
	usbmuxd_log(LL_DEBUG, "[OUT][AOA] dev=%d sport=%d dport=%d seq=%d ack=%d window=%d len=%d",
		    conn->dev->id, conn->sport, conn->dport, conn->tx_seq, conn->tx_ack, conn->tx_win, length);
...
* Android AOA Handle
*/
static void device_aoa_input(struct mux_device *dev,  unsigned char *payload, uint32_t payload_length)
{
	struct mux_connection *conn = NULL;

	usbmuxd_log(LL_DEBUG, "[IN] dev=%d  len=%d", dev->id, payload_length);
	...
	// Find the connection on this device that has the right sport and dport
	FOREACH(struct mux_connection *lconn, &dev->connections) {
		if(lconn->state == CONN_CONNECTED &&
			lconn->sport == AOA_TCP_PORT && lconn->dport == AOA_TCP_PORT){
			conn = lconn;
			break;
		}
	} ENDFOREACH 
...
int device_add(struct usb_device *usbdev)
{
	...
	if(dev->usbdev->type == USB_IOS){
		...
		usbmuxd_log(LL_ERROR, "Error USB Type %d", dev->usbdev->type);
		return -1;
	}
	pthread_mutex_lock(&device_list_mutex);
